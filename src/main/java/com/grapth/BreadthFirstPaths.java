...
public class BreadthFirstPaths {
    ...
    // breadth-first search from a single source // NO RECURSION !!!!
    private void bfs(Graph graph, int s) {
        ...
        while (!q.isEmpty()) {
            int next = q.dequeue();
            for (int each : graph.getNodeAdjacents(next)) {
                if (!marked[each]) {
                    edgeTo[each] = next;
                    distTo[each] = distTo[next] + 1;
                    marked[each] = true;
                    q.enqueue(each);
                }
            }
    ...
    private void bfs(Graph G, Iterable<Integer> sources) {
        ...
        while (!q.isEmpty()) {
            int v = q.dequeue();
            for (int w : G.getNodeAdjacents(v)) {
                if (!marked[w]) {
                    edgeTo[w] = v;
                    distTo[w] = distTo[v] + 1;
                    marked[w] = true;
     ...
     *
     * @param toNode the vertex
     * @return the sequence of vertices on a shortest path, as an Iterable
     */
    public Iterable<Integer> pathTo(int toNode) {
        if (!hasPathTo(toNode)) return null;
        Stack<Integer> path = new Stack<Integer>();
        int current;
        for (current = toNode; distTo[current] != 0; current = edgeTo[current])
            path.push(current);
        path.push(current);
        return path;
    }
    ...
    public static void main(String[] args) {
        In in = new In("/home/adelin/learn/jads/src/main/java/com/grapth/mediumGrapth.txt");
